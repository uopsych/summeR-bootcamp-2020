...
You might also notice that each of the arguments have a value after the `=` in the documentation. These values are the *defaults*; they are what the arguments will be set to if you don't specify them. 
...
If we look at the help documention, you can see `x` is the data from which to calculate a mean.
...
This sort of makes sense - the mean of `vec_na` in its entirity is unknown, since we don't know what the `NA` value is. That's why you have to remove `NA`'s before running calculations by setting `na.rm = TRUE`
...
1. In your RStudio console, look up the help documentation for the `import()` function. What do you see?
